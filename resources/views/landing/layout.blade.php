...
<body class="font-sans antialiased bg-background text-foreground">
    <a href="#landing-main" class="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-primary focus:text-primary-foreground focus:rounded-lg focus:shadow-lg">Lompat ke konten utama</a>

    <nav x-data="{ open: false, scrolled: false }" x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 20 })" class="fixed w-full z-50 transition-all duration-300" :class="scrolled ? 'bg-card/80 backdrop-blur-xl border-b border-border shadow-sm' : 'bg-transparent'" role="navigation" aria-label="Navigasi utama">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16 md:h-20">
                ...
                <div class="hidden lg:flex items-center space-x-1">
                    ...
                    }" {{ request()->routeIs('landing.index') ? 'aria-current="page"' : '' }}>Beranda</a>
                    ...
                    }" {{ request()->routeIs('landing.info') ? 'aria-current="page"' : '' }}>Info PPDB</a>
                    ...
                    }" {{ request()->routeIs('landing.jalur') ? 'aria-current="page"' : '' }}>Jalur Pendaftaran</a>
                    ...
                    }" {{ request()->routeIs('landing.pengumuman*') ? 'aria-current="page"' : '' }}>Pengumuman</a>
                    ...
                    }" {{ request()->routeIs('landing.faq') ? 'aria-current="page"' : '' }}>FAQ</a>
                    ...
                    }" {{ request()->routeIs('landing.kontak') ? 'aria-current="page"' : '' }}>Kontak</a>
                </div>
                ...
                <button @click="open = !open" class="lg:hidden p-2 rounded-lg" :class="scrolled ? 'text-foreground hover:bg-accent' : 'text-white/80 hover:text-white hover:bg-white/10'" aria-label="Buka menu navigasi" aria-expanded="false">
        ...
        <div :class="{'block': open, 'hidden': !open}" class="lg:hidden bg-card border-t border-border shadow-xl">
            <div class="px-4 py-3 space-y-1">
                <a href="{{ route('landing.index') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium {{ request()->routeIs('landing.index') ? 'text-primary bg-primary/10' : 'text-foreground hover:bg-accent' }}" {{ request()->routeIs('landing.index') ? 'aria-current="page"' : '' }}>Beranda</a>
                <a href="{{ route('landing.info') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium {{ request()->routeIs('landing.info') ? 'text-primary bg-primary/10' : 'text-foreground hover:bg-accent' }}" {{ request()->routeIs('landing.info') ? 'aria-current="page"' : '' }}>Info PPDB</a>
                <a href="{{ route('landing.jalur') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium {{ request()->routeIs('landing.jalur') ? 'text-primary bg-primary/10' : 'text-foreground hover:bg-accent' }}" {{ request()->routeIs('landing.jalur') ? 'aria-current="page"' : '' }}>Jalur Pendaftaran</a>
                <a href="{{ route('landing.pengumuman') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium {{ request()->routeIs('landing.pengumuman*') ? 'text-primary bg-primary/10' : 'text-foreground hover:bg-accent' }}" {{ request()->routeIs('landing.pengumuman*') ? 'aria-current="page"' : '' }}>Pengumuman</a>
                <a href="{{ route('landing.faq') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium {{ request()->routeIs('landing.faq') ? 'text-primary bg-primary/10' : 'text-foreground hover:bg-accent' }}" {{ request()->routeIs('landing.faq') ? 'aria-current="page"' : '' }}>FAQ</a>
                <a href="{{ route('landing.kontak') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium {{ request()->routeIs('landing.kontak') ? 'text-primary bg-primary/10' : 'text-foreground hover:bg-accent' }}" {{ request()->routeIs('landing.kontak') ? 'aria-current="page"' : '' }}>Kontak</a>
    ...
    <footer class="bg-card border-t border-border" role="contentinfo">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                <div class="lg:col-span-2">
                    ...
                    <div class="flex space-x-4 mt-6">
                        <a href="#" class="w-9 h-9 rounded-full bg-secondary hover:bg-primary hover:text-primary-foreground flex items-center justify-center transition-all duration-200 text-muted-foreground" aria-label="Facebook">
                        ...
                        <a href="#" class="w-9 h-9 rounded-full bg-secondary hover:bg-primary hover:text-primary-foreground flex items-center justify-center transition-all duration-200 text-muted-foreground" aria-label="YouTube">
                            <svg aria-hidden="true" class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                        </a>
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <ul class="space-y-3 text-muted-foreground">
                        <li class="flex items-start space-x-2.5">
                            <svg aria-hidden="true"  class="w-4 h-4 mt-0.5 text-primary shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" ><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                            <span class="text-sm">Jl. Pendidikan No. 123, Kota Pendidikan</span>
                        </li>
                        <li class="flex items-center space-x-2.5">
                            <svg aria-hidden="true"  class="w-4 h-4 text-primary shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" ><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                            <span class="text-sm">info@ppdb-online.sch.id</span>
                        </li>
                        <li class="flex items-center space-x-2.5">
                            <svg aria-hidden="true"  class="w-4 h-4 text-primary shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" ><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
