                                        ...
                                        <div>
                                            ...
                                            @if ($pembayaran->bukti_bayar)
                                                <div class="mt-2 flex items-center gap-3">
                                                    ...
                                                    @if ($pembayaran->status === 'diverifikasi' || $pembayaran->status === 'lunas')
                                                        <a href="{{ route('tagihan.receipt', $pembayaran) }}" target="_blank" class="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1">
                    ...
                    @else
                        <div class="text-center py-6">
                            <div class="w-14 h-14 mx-auto mb-3 rounded-2xl bg-muted flex items-center justify-center">
                                <svg aria-hidden="true"   class="w-7 h-7 text-muted-foreground/60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            </div>
                        ...
                        <form action="{{ route('pembayaran.store', $tagihan) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="file" name="bukti_bayar" id="bukti_bayar" accept="image/*,.pdf" required
                                    class="block w-full text-sm text-muted-foreground file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 cursor-pointer" aria-label="Upload Bukti Bayar">
                            ...
                            <div>
                                ...
                                <textarea name="catatan" id="catatan" rows="2" class="block w-full rounded-xl border-input bg-background text-foreground shadow-sm focus:border-ring focus:ring-ring text-sm" placeholder="Tambahkan catatan jika diperlukan" aria-label="Catatan Pembayaran"></textarea>
                            </div>
