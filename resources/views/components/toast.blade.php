...
<script>
    ...
    @if(session('success'))
        document.addEventListener('DOMContentLoaded', () => window.toast("{{ session('success') }}", 'success'));
    @endif
    @if(session('error'))
        document.addEventListener('DOMContentLoaded', () => window.toast("{{ session('error') }}", 'error'));
    @endif
</script>
@endonce